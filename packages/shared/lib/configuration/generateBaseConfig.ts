...
const baseConfig = (baseConfigInputs: BaseConfigInputs) => {
    ...
    return {
        ...
        locationForFile: locationForFile.length ? locationForFile : './testAssets/appLocators.js',
    }
};
...
export const generateBaseConfig = (baseConfigInputs: BaseConfigInputs) => {
    ...
    fs.writeFile(pathAtCwd, 'module.exports =', {encoding: 'utf-8'}, (err) => {
        ...
        fs.appendFile(pathAtCwd, util.inspect(baseConfig(baseConfigInputs)), () => {
            if (err) {
                console.error('@dlg', 'error creating configuration content.')
                console.error(err);
            }

            console.log('@dlg - successfully created dlg.config.js at', pathAtCwd);
        })
    })
}
